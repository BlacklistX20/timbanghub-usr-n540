...
-- VERSI: DATABASE ONLINE
-- =========================================================
-- File ini untuk instalasi BARU (database kosong). Beda dari
-- versi LOKAL: tabel di sini TIDAK punya kolom `synced_at`
-- (kolom itu cuma relevan di sisi lokal, untuk menandai data
-- mana yang sudah/belum dikirim ke sini).
--
-- Satu kolom baru yang TETAP ada di sini (sama seperti lokal):
-- `sync_id` di scale_status_logs - dibutuhkan di KEDUA sisi
-- supaya proses insert dari sync worker bisa idempoten (aman
-- di-retry tanpa data dobel).
-- =========================================================
